...
ivec dec2bin(int length, int index)
{
  int i, bintemp = index;
  ivec temp(length);
...
ivec dec2bin(int index, bool msb_first)
{
...
int bin2dec(const ivec &inbvec, bool msb_first)
{
  int i, temp = 0;
  int sizebvec = inbvec.length();
  if (msb_first) {
    for (i = 0; i < sizebvec; i++) {
      temp += pow2i(sizebvec - i - 1) * inbvec(i);
    }
  }
  else {
    for (i = 0; i < sizebvec; i++) {
      temp += pow2i(i) * inbvec(i);
    }
  }
  return temp;
}

ivec oct2bin(const ivec &octalindex, short keepzeros)
{
...
ivec bin2oct(const ivec &inbits)
{
...
ivec bin2pol(const ivec &inbvec)
{
  return 1 - 2*inbvec;
}

ivec pol2bin(const ivec &inpol)
{
  return ((1 -inpol) / 2);
}
