...
Vec<Num_T> operator*(Num_T t, const Vec<Num_T> &v);
  ...
  //! Elementwise reminder of vector and scalar
template<class Num_T>
Vec<Num_T> operator%(const Vec<Num_T> &v, Num_T t);
  //! Elementwise reminder of vector and scalar
template<class Num_T>
...
/*!
  ...
  prepared for \c bin (REMOVED:HARI), \c short, \c int, \c double, and \c complex<double>
...
public:
  ...
  Vec<bool> operator==(Num_T t) const;
  Vec<bool> operator!=(Num_T t) const;
...
template<>
Vec<double>& Vec<double>::operator%=(double t)
{
std::cout<<"\nVec<double>operator%=(): is not defined for double\n";
(void)t;
return *this;
}

template<>
Vec<std::complex<double>>& Vec<std::complex<double>>::operator%=(std::complex<double> t)
{
std::cout<<"\nVec<std::complex<double>>operator%=(): is not defined for std::complex<double>\n";
(void)t;
return *this;
}
...
template<class Num_T>
Vec<bool> Vec<Num_T>::operator==(Num_T t) const
{
  it_assert_debug(datasize > 0, "Vec<>::operator==(): Wrong size");
  Vec<bool> temp(datasize);
  for (int i = 0; i < datasize; i++)
    temp(i) = (data[i] == t);
  return temp;
}
...
//! \cond
template<>
Vec<bool> Vec<std::complex<double> >::operator<(std::complex<double>) const;
//! \endcond
...
//! \cond
template<>
ITPP_EXPORT Vec<bool> Vec<std::complex<double> >::operator<=(std::complex<double>) const;
//! \endcond
...
{
  (void)s; //-Werror=unused-parameter
