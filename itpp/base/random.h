...
public:
  ...
  //! Get a sample vector.
  ivec operator()(int n) { ivec temp(n); sample_vector(n, temp); return temp; }
  //! Get a sample matrix.
  imat operator()(int h, int w) { imat temp(h, w); sample_matrix(h, w, temp); return temp; }
  //! Get a sample
  int sample() { return RNG.genrand_close_open() < p ? int(1) : int(0); }
  //! Get a sample vector.
  void sample_vector(int size, ivec &out) {
    out.set_size(size, false);
    for(int i = 0; i < size; i++) out(i) = sample();
  }
  //! Get a sample matrix.
  void sample_matrix(int rows, int cols, imat &out) {
...
inline int randb(void) { Bernoulli_RNG src; return src.sample(); }
//! Generates a random bit vector (equally likely 0s and 1s)
inline void randb(int size, ivec &out) { Bernoulli_RNG src; src.sample_vector(size, out); }
//! Generates a random bit vector (equally likely 0s and 1s)
inline ivec randb(int size) { ivec temp; randb(size, temp); return temp; }
//! Generates a random bit matrix (equally likely 0s and 1s)
inline void randb(int rows, int cols, imat &out) { Bernoulli_RNG src; src.sample_matrix(rows, cols, out); }
//! Generates a random bit matrix (equally likely 0s and 1s)
inline imat randb(int rows, int cols) { imat temp; randb(rows, cols, temp); return temp; }
